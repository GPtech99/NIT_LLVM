...
/* method model_base#MConcern#parent_concern for (self: MConcern): nullable MConcern */
val* nit___nit__MConcern___parent_concern(val* self) {
val* var /* : nullable MConcern */;
const char* var_class_name;
var_class_name = self == NULL ? "null" : self->type->name;
PRINT_ERROR("Runtime error: Abstract method `%s` called on `%s`", "parent_concern", var_class_name);
PRINT_ERROR(" (%s:%d)\n", FILE_nit__model_base, 72);
fatal_exit(1);
RET_LABEL:;
return var;
}
...
var_other = p0;
{
var2 = ((long(*)(val* self))(self->class->vft[COLOR_nit__model_base__MVisibility__level]))(self); /* level on <self:MVisibility>*/
}
{
var3 = ((long(*)(val* self))(var_other->class->vft[COLOR_nit__model_base__MVisibility__level]))(var_other); /* level on <var_other:MVisibility>*/
}
{
